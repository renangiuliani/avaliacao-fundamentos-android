...
public class ServiceOrderActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(extras == null){
            ...
            final String[] dateTimeArray = dateTimeFormat.format(mServiceOrder.getDate()).split("[;]");
            mEditTextDate.setText(dateTimeArray[0]);
            mEditTextTime.setText(dateTimeArray[1]);
            final DecimalFormat decimalFormat =  AppUtil.get(NumberFormat.getInstance(Locale.US));
    ...
    private boolean verifyServiceOrderValue() {
        ...
        if (!TextUtils.isEmpty(valueText)) {
            final Pattern pattern = Pattern.compile("[0-9]+([.][0-9]{2,2})");
